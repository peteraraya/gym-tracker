...
export function FloatingAIAssistant() {
  ...
  const [modalAction, setModalAction] = useState<'search' | 'suggest' | null>(null);
  const [modalInput, setModalInput] = useState('');
  const [modalSelection, setModalSelection] = useState<number | null>(null);
  const [customSplit, setCustomSplit] = useState<'fullbody' | 'upper_lower' | 'ppl'>('fullbody');
  const [customDays, setCustomDays] = useState<number>(3);
  const [customEmphasis, setCustomEmphasis] = useState<'fuerza'|'hipertrofia'|'resistencia'>('hipertrofia');
  ...
  const handleQuickProgress = () => {
    ...
    if (!sessions || sessions.length === 0) {
      addAssistantMessage('📊 Aún no tienes sesiones registradas. ¡Empieza a entrenar!');
      return;
    }
    const total = sessions.length;
    const avg = recent.reduce((s, x) => s + (x.totalDuration || 0), 0) / Math.max(1, recent.length);
    addAssistantMessage(`📊 Análisis rápido:\n\n✅ Sesiones: ${total}\n⏱️ Duración promedio últimas ${recent.length}: ${Math.floor(avg / 60)} min`);
  };
  ...
  const performSearch = (q: string) => {
    if (!q) return;
    const found = EXERCISE_DATABASE.find(ex => ex.name.toLowerCase().includes(q.toLowerCase()));
    if (found) {
      addAssistantMessage(`💪 **${found.name}**\n\n${found.description || 'Sin descripción'}\n\n🎯 Grupo: ${found.muscleGroup}`);
    } else {
      addAssistantMessage('No encontré un ejercicio con ese nombre. Intenta otro término.');
    }
  };
  ...
  const performCustomSuggest = (split: typeof customSplit, days: number, emphasis: typeof customEmphasis) => {
    ...
    if (split === 'fullbody') {
      text += 'Ejemplo por día: 2 compuestos principales + 1 accesorio. 3x8-12 cada uno.';
    } else if (split === 'upper_lower') {
      text += 'Upper: 3-4 ejercicios compuestos. Lower: 3-4 ejercicios con énfasis en piernas.';
    } else {
      text += 'Push/Pull/Legs: divide en 3 días centrados en empuje, tracción y piernas.';
    }
  ...
  const handleWarmup = () => {
    ...
    if (candidates.length === 0) {
      addAssistantMessage('Calentamiento sugerido: movilidad general + 5-10 min cardio ligero.');
    } else {
      addAssistantMessage(`Calentamiento para ${target}: ${candidates.map(c => c.name).join(', ')}\nRealiza series ligeras de activación antes de las series de trabajo.`);
    }
  };
  ...
  const generateResponse = async (userMessage: string): Promise<string> => {
    ...
    if (trainerMode) {
      try {
        const trainerReply = getTrainerReply(userMessage, trainerLevel, sessions);
  ...
  const filteredExercises = (modalAction === 'search' && modalInput.trim())
    ? EXERCISE_DATABASE.filter(ex => ex.name.toLowerCase().includes(modalInput.trim().toLowerCase())).slice(0, 5)
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 flex flex-col h-full overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-purple-600 to-blue-600 p-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <span className="text-white text-xs">{trainerMode ? 'Entrenador' : 'Entrenador'}</span>
            </button>
            <button
        ...
        {!isMinimized && (
          <>
            ...
            <div className="p-3 border-b border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900">
              <div className="flex items-center gap-2">
                <label htmlFor="quick-action" className="sr-only">Acciones rápidas</label>
                <select
                  id="quick-action"
                  value={''}
                  onChange={(e) => {
                    ...
                    // reset selection
                    try { (e.target as HTMLSelectElement).selectedIndex = 0; } catch (err) {}
                  }}
                ...
                {trainerMode && (
                  <select
                    value={trainerLevel}
                    onChange={(e) => setTrainerLevel(e.target.value as TrainerLevel)}
            ...
            {/* Modal for prompts */}
            <Modal isOpen={modalOpen} onClose={() => { setModalOpen(false); setModalSelection(null); setModalInput(''); setModalAction(null); }} title={modalAction === 'search' ? 'Buscar ejercicio' : 'Sugerir rutina'}>
              <div className="space-y-3">
                {modalAction === 'search' ? (
                  <>
                    <p className="text-sm text-gray-700">Buscar o elige un ejercicio de la lista (pulsa el número):</p>
                    <input
                      value={modalInput}
                      onChange={(e) => { setModalInput(e.target.value); setModalSelection(null); }}
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {filteredExercises.map((ex, idx) => (
                        <button
                          key={ex.name}
                          onClick={() => { setModalSelection(idx + 1); }}
                        ...
                        >
                          <strong className="mr-2">{idx + 1}.</strong> {ex.name} <span className="text-xs text-gray-500">{ex.muscleGroup || ''}</span>
                        </button>
                      ))}
                  ...
                  <>
                    <p className="text-sm text-gray-700">Selecciona el objetivo:</p>
                    ...
                    {modalSelection === 7 && (
                      <div className="mt-3 space-y-2 p-2 border-t border-gray-100 dark:border-gray-800">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <select value={customDays} onChange={(e) => setCustomDays(Number(e.target.value))} className="px-2 py-1 rounded-lg bg-gray-100 dark:bg-gray-800 border">
                ...
                <div className="flex justify-end gap-2">
                  <button
                    onClick={() => { setModalOpen(false); setModalSelection(null); setModalInput(''); setModalAction(null); }}
                    ...
                    onClick={() => {
                        if (modalAction === 'search') {
                          ...
                          if (q) {
                            ...
                            if (!sel) { toast.info('Selecciona una opción o escribe para buscar'); return; }
                            const ex = filteredExercises[sel - 1];
                            if (ex) {
                              addAssistantMessage(`💪 **${ex.name}**\n\n${ex.description || 'Sin descripción'}\n\n🎯 Grupo: ${ex.muscleGroup}`);
                            }
                          }
                        }
                      if (modalAction === 'suggest') {
                        const sel = modalSelection;
                        if (!sel) { toast.info('Selecciona una opción'); return; }
                        ...
                        if (sel === 7) performCustomSuggest(customSplit, customDays, customEmphasis);
                      }
                      setModalOpen(false);
                      setModalInput('');
