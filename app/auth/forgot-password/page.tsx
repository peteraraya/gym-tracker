  ...
  const onSubmit = async (values: ForgotPasswordFormData) => {
    try {
      ...
      if (error) {
        ...
        const message = status === 429 || /rate limit|too many requests|rate_limited/i.test(raw)
          ? (t('emailRateLimit') || 'Se ha excedido el límite de envío de correos. Intenta de nuevo más tarde.')
          : (raw || t('sendResetError') || 'Error al enviar el enlace. Intenta nuevamente más tarde.');
      ...
      if (typeof window !== 'undefined') {
        try {
          ...
          localStorage.setItem(`${COOLDOWN_KEY_PREFIX}${values.email}`, String(expiresAt));
        } catch {}
      }
    } catch {
