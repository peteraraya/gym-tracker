...
import { authSchema, getRegisterSchema, type AuthFormData } from '@/lib/schemas/auth';
...
export default function AuthPage() {
  ...
  const schema = useMemo(() => (isLogin ? authSchema : getRegisterSchema()), [isLogin]);
  ...
  const onSubmit = useCallback(async (values: AuthFormData) => {
    try {
      if (isLogin) {
        ...
        if (error) {
          const raw = getMessage(error);
          const isDuplicate = /already exists|duplicate|already registered/i.test(raw);
