 ...
 */
export function calculateSessionVolume(exercises: ExerciseWithSets[] | undefined): number {
    if (!exercises || !Array.isArray(exercises)) return 0;

    return exercises.reduce((total, ex) => {
        if (!ex.actualReps || !Array.isArray(ex.actualReps)) return total;
        return total + ex.actualReps.reduce((repTotal, reps, idx) => {
 ...
 */
export function calculateTotalSets(exercises: ExerciseWithSets[] | undefined): number {
    if (!exercises || !Array.isArray(exercises)) return 0;
...
export function calculateStreak(sessions: WorkoutSession[]): number {
    ...
    if (!isSameDay(uniqueDates[0], today) && !isPreviousDay(uniqueDates[0], today)) {
        return 0;
    }
